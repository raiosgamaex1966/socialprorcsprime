...
const PageEvents = ({ pageId, isOwner }: PageEventsProps) => {
  const { user } = useAuth();
  ...
  const handleCreate = async (values: EventFormValues) => {
    ...
    setShowForm(false);
    queryClient.invalidateQueries({ queryKey: ["page-events", pageId] });
  ...
  const handleDelete = async (eventId: string) => {
    try {
      const { error } = await supabase.from("group_events" as any).delete().eq("id", eventId);
      if (error) throw error;
      queryClient.invalidateQueries({ queryKey: ["page-events", pageId] });
  ...
  const handleDeleteSeries = async (parentEventId: string) => {
    try {
      await supabase.from("group_events" as any).delete().eq("parent_event_id", parentEventId);
      const { error } = await supabase.from("group_events" as any).delete().eq("id", parentEventId);
      if (error) throw error;
      queryClient.invalidateQueries({ queryKey: ["page-events", pageId] });
        ...
        <>
          ...
          {!events?.length && (
            <p className="text-sm text-muted-foreground text-center py-4">
              Nenhum evento ainda.{isOwner ? " Crie um!" : ""}
            </p>
          )}
        </>
      )}
